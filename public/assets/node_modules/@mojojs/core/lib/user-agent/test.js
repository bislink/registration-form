...
function addNodeAssertMethods(tapInstance) {
    tapInstance.strictEqual = tapInstance.equal.bind(tapInstance);
    tapInstance.notStrictEqual = tapInstance.not.bind(tapInstance);
    tapInstance.doesNotMatch = tapInstance.notMatch.bind(tapInstance);
    tapInstance.deepStrictEqual = tapInstance.same.bind(tapInstance);
}
/**
...
export class TestUserAgent extends MockUserAgent {
    ...
    textUnlike(selector, regex) {
        this.assert('doesNotMatch', [this._html.at(selector)?.text() ?? '', regex], `no similar match for selector "${selector}"`, this.textLike);
        return this;
    }
    /**
